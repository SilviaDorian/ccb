...
/**
 * GET /api/users/team (or /api/users/downlines)
 * Returns masked referral tree & team performance stats
 */
router.get('/team', requireAuth, async (req, res) => {
  try {
    const userId = req.user.id;

    // Fetch referral tree members
    const { data: referrals, error: refError } = await supabaseAdmin
      .from('users')
      .select('id, phone, first_name, created_at, tasks_completed, status')
      .eq('referred_by', userId)
      .order('created_at', { ascending: false });

    if (refError) {
      return error(res, 'Failed to fetch referral records', 500);
    }

    // Mask phone numbers for user privacy
    const maskedReferrals = (referrals || []).map(ref => ({
      ...ref,
      phone: ref.phone ? `${ref.phone.slice(0, 4)}***${ref.phone.slice(-3)}` : 'N/A'
    }));

    return success(res, {
      referral_code: req.user.referral_code,
      total_referrals: req.user.referral_count || maskedReferrals.length,
      total_referral_earnings: req.user.referral_earnings || 0.00,
      downlines: maskedReferrals
    });
